...
<!-- 脚本接口版本：screenshot.py v2.1 / gen_flowmap.py v5.0 / publish_to_iwiki.py v1.2 -->
...
> **⚙️ 自动安装说明**：本 Skill 在首次运行时会自动检测并安装所需的 Python 依赖（`requests` 和 `playwright`）。这些依赖用于 iWiki MCP API 调用和 HTML 原型自动截图。如果您的环境中已安装这些库，此步骤将自动跳过，不会产生任何额外操作。
...
2. **检查 `requests` 库**：`python3 -c "import requests"` 
   - 成功 → ✅ 继续
   - 失败 → 自动执行 `pip install requests`（用于 iWiki MCP API 调用，见 frontmatter dependencies 声明）
3. **检查 `screenshot.py`**：确认 `{WORKSPACE}/screenshot.py` 存在
   - 存在 → ✅ 继续
   - 不存在 → ⚠️ 提示：自动截图功能不可用，可手动截图后放入 `prototype/screenshots/`
4. **检查 Playwright**：`python3 -c "from playwright.sync_api import sync_playwright"`
   - 成功 → ✅ 继续
   - 失败 → 自动执行 `pip install playwright && python3 -m playwright install chromium`（用于 HTML 原型自动截图，见 frontmatter dependencies 声明）
   - 如果安装失败（如网络超时），告知用户：
     ```
     ⚠️ 截图工具安装失败，PRD文档生成不受影响，原型截图将在下次重试。
     可手动截图后放入 prototype/screenshots/ 目录。
     ```
     并继续执行后续Step（跳过截图相关步骤）。
...
**截图重做步骤**（仅对受影响的页面）：
```bash
# 1. 确保PRD预览服务已启动（如 Step 4 已启动则跳过）
PORT=8888
while lsof -i:$PORT > /dev/null 2>&1; do PORT=$((PORT+1)); done
python3 {WORKSPACE}/prd_preview_server.py --port $PORT &
...
<!-- 脚本接口版本：screenshot.py v2.1 / gen_flowmap.py v5.0 / publish_to_iwiki.py v1.2 / prd_preview_server.py v1.0 -->
